...
# Based on https://github.com/Microsoft/azure-pipelines-tasks/blob/master/Tasks/PublishBuildArtifactsV1/Invoke-Robocopy.ps1
...
# Print the ##command.
[System.Console]::WriteLine("##[command]robocopy.exe `"$Source`" `"$Target`" $Flags")
$robocopy = [Scriptblock]::Create("robocopy.exe `"$Source`" `"$Target`" $Flags")
