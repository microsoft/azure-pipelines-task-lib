...
describe('Command Tests', function () {
    ...
    it('constructs', function (done) {
        ...
        const tc = new tcm.TaskCommand('some.cmd', { foo: 'bar' }, 'a message');
    ...
    it('toString escapes message', function (done) {
        ...
        const tc = new tcm.TaskCommand('some.cmd', { foo: 'bar' }, 'cr \r lf \n crlf \r\n eom ] ;');
        assert(tc, 'TaskCommand constructor works');
        const cmdStr = tc.toString();
    ...
    it('toString handles non string value in properties', function (done) {
        ...
        const tc = new tcm.TaskCommand('some.cmd', { foo: ['bar', 'baz'] }, 'cr \r lf \n crlf \r\n eom ] ;');
        assert(tc, 'TaskCommand constructor works');
        const cmdStr = tc.toString();
    ...
    it('toString escapes properties', function (done) {
        ...
        const tc = new tcm.TaskCommand('some.cmd', { foo: ';=\r=\n%3B' }, 'dog');
        assert(tc, 'TaskCommand constructor works');
        const cmdStr = tc.toString();
    ...
    it('toString writes isOutput', function (done) {
        ...
        const tc = new tcm.TaskCommand('task.setvariable', { variable: 'bar', isOutput: 'true' }, 'dog');
        assert(tc, 'TaskCommand constructor works');
        const cmdStr = tc.toString();
    ...
    it('handles null properties', function (done) {
        ...
        const tc = new tcm.TaskCommand('some.cmd', null, 'a message');
    ...
    it('parses cmd with no properties', function (done) {
        const cmdStr = '##vso[basic.command]messageVal';

        const tc = tcm.commandFromString(cmdStr);
    ...
    it('parses basic cmd with values', function (done) {
        const cmdStr = '##vso[basic.command prop1=val1;]messageVal';
    ...
    it('parses basic cmd with multiple properties no trailing semi', function (done) {
        const cmdStr = '##vso[basic.command prop1=val1;prop2=val2]messageVal';
    ...
    it('parses values with spaces in them', function (done) {
        const cmdStr = '##vso[task.setvariable variable=task variable;]task variable set value';
    ...
    it('parses and unescapes message', function (done) {
        const cmdStr = '##vso[basic.command]cr %0D lf %0A crlf %0D%0A eom ] ;';
    ...
    it('parses and unescapes properties', function (done) {
        const cmdStr = '##vso[basic.command foo=%3B=%0D=%0A%AZP253B;]dog';
    ...
    it('handles empty properties', function (done) {
        ...
        const tc = new tcm.TaskCommand('some.cmd', {}, 'a message');
        assert.equal(tc.toString(), '##vso[some.cmd]a message');
