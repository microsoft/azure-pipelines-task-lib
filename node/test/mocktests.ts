...
describe('Mock Tests', function () {
    ...
    it('mock-task exports every function in task', (done) => {
        for (const memberName of Object.keys(tl) as Array<keyof typeof tl>) {
            const member = tl[memberName];

            if (typeof member === 'function') {
                const mockMember = mt[memberName as keyof typeof mt];
    ...
    it('match not mocked', (done) => {
        const actual: string[] = (mt as any).match(
            [
                '/foo',
                '/bar',
                '/baz',
    ...
    it('filter not mocked', (done) => {
        ...
        const actual: string[] = list.filter((mt as any).filter('/ba[rz]'));
        assert.deepEqual(actual, ['/bar', '/baz']);
    ...
    it('Mocks findMatch results', (done) => {
        ...
        mt.setAnswers(a);
        const matches: string[] = mt.findMatch('/default-root', '/ba[rz]');
    ...
    it('Mock toolRunner returns success code', async () => {
        ...
        const tool: mtr.ToolRunner = mt.tool('/usr/local/bin/atool');
        tool.arg('--arg');
        tool.arg('foo');
        const rc: number = await tool.exec(<mtr.IExecOptions>{});
