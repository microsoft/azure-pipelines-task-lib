...
describe('Input Tests', function () {
    ...
    it('gets input value', function (done) {
        ...
        const inval = tl.getInput('UnitTestInput', true);
    ...
    it('should clear input envvar', function (done) {
        ...
        const inval = tl.getInput('UnitTestInput', true);
        assert.equal(inval, 'test value');
        assert.ok(!process.env['INPUT_UNITTESTINPUT'], 'input envvar should be cleared');
    ...
    it('required input throws', function (done) {
        ...
        try {
            const inval = tl.getInput('SomeUnsuppliedRequiredInput', true);
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'req input should have not have worked');
    ...
    it('gets input value', function (done) {
        ...
        const inval = tl.getInputRequired('UnitTestInput');
    ...
    it('required input throws', function (done) {
        ...
        try {
            tl.getInputRequired('SomeUnsuppliedRequiredInput');
            worked = true;
        } catch (err) { }
    ...
    it('gets a variable', function (done) {
        ...
        const varVal = tl.getVariable('Build.Repository.Name');
    ...
    it('gets a variable with special characters', (done) => {
        ...
        const expected = 'Value of const with special chars';
        process.env['HELLO_DOT_DOT2_SPACE_SPACE2'] = expected;
        im._loadData();

        const varVal = tl.getVariable('Hello.dot.dot2 space space2');
    ...
    // setVariable tests
    it('sets a variable as an env const', function (done) {
        this.timeout(1000);

        tl.setVariable('Build.Repository.Uri', 'test value');
        const varVal = process.env['BUILD_REPOSITORY_URI'];
        assert.equal(varVal, 'test value');

        done();
    });

    it('sets a variable with special chars as an env const', (done) => {
        this.timeout(1000);

        const expected = 'Set value of const with special chars';
        tl.setVariable('Hello.dot.dot2 space space2', expected);
        const varVal = process.env['HELLO_DOT_DOT2_SPACE_SPACE2'];
        assert.equal(varVal, expected);
    ...
    it('sets and gets a variable', function (done) {
        ...
        tl.setVariable('UnitTestVariable', 'test const value');
        const varVal = tl.getVariable('UnitTestVariable');
        assert.equal(varVal, 'test const value');
    ...
    it('sets and gets an output variable', function (done) {
        ...
        tl.setVariable('UnitTestVariable', 'test const value', false, true);
        const varVal = tl.getVariable('UnitTestVariable');
        assert.equal(varVal, 'test const value');
    ...
    it('sets and gets a secret variable', function (done) {
        ...
        const varVal = tl.getVariable('My.Secret.Var');
        assert.equal(varVal, 'test secret value');

        done();
    });

    it('does not set a secret variable as an env const', function (done) {
        this.timeout(1000);

        delete process.env['MY_SECRET_VAR'];
        tl.setVariable('My.Secret.Var', 'test secret value', true);
        const envVal = process.env['MY_SECRET_VAR'];
        assert.ok(!envVal, 'env const should not be set');

        done();
    });

    it('removes env const when sets a secret variable', function (done) {
        this.timeout(1000);
    ...
    it('does not allow a secret variable to become a public variable', function (done) {
        ...
        const vars: tl.VariableInfo[] = tl.getVariables();
    ...
    it('does not allow setting a multi-line secret variable', function (done) {
        ...
        assert.ok(failed, 'Should have failed setting a secret variable with a carriage return');
        ...
        assert.ok(failed, 'Should have failed setting a secret variable with a line feed');
    ...
    it('does not allow setting a multi-line secret', function (done) {
        ...
        assert.ok(failed, 'Should have failed setting a secret with a carriage return');
        ...
        assert.ok(failed, 'Should have failed setting a secret with a line feed');
    ...
    it('gets public variables from initial load', function (done) {
        ...
        assert.ok(vars, 'variables should not be undefined or null');
    ...
    it('gets an endpoint url', function (done) {
        ...
        const url = tl.getEndpointUrl('id1', true);
    ...
    it('gets a required endpoint url', function (done) {
        ...
        const url = tl.getEndpointUrlRequired('id1');
        assert.equal(url, 'http://url', 'url should match');
    ...
    it('required endpoint url throws', function (done) {
        ...
        try {
            const url = tl.getEndpointUrlRequired('SomeUnsuppliedRequiredInput');
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'req endpoint url should have not have worked');
    ...
    it('gets an endpoint auth', function (done) {
        ...
        const auth = tl.getEndpointAuthorization('id1', true);
        assert.ok(auth, 'should return an auth obj');
    ...
    it('should clear auth envvar', function (done) {
        ...
        const auth = tl.getEndpointAuthorization('id1', true);
        assert.ok(auth, 'should return an auth obj');
        assert.ok(auth['parameters']['param1'] === 'val1', 'should be correct object');
        assert.ok(!process.env['ENDPOINT_AUTH_id1'], 'should clear auth envvar');
    ...
    it('gets endpoint auth scheme', function (done) {
        ...
        const data = tl.getEndpointAuthorizationScheme('id1', true);
        assert.ok(data, 'should return a string value');
        assert.equal(data, 'scheme1', 'should be correct scheme');
        assert.ok(!process.env['ENDPOINT_AUTH_SCHEME_id1'], 'should clear auth envvar');
    ...
    it('gets undefined if endpoint auth scheme is not set', function (done) {
        ...
        const data = tl.getEndpointAuthorizationScheme('id1', true);
        assert.ok(!data, 'should be undefined when auth scheme is not set');
    ...
    it('gets required endpoint auth scheme', function (done) {
        ...
        const data = tl.getEndpointAuthorizationSchemeRequired('id1');
        assert.ok(data, 'should return a string value');
        assert.equal(data, 'scheme1', 'should be correct scheme');
        assert.ok(!process.env['ENDPOINT_AUTH_SCHEME_id1'], 'should clear auth envvar');
    ...
    it('required endpoint auth scheme throws', function (done) {
        ...
        try {
            tl.getEndpointAuthorizationSchemeRequired('SomeUnsuppliedRequiredInput');
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'req endpoint auth scheme should have not have worked');
    ...
    it('gets endpoint auth parameters', function (done) {
        ...
        const data = tl.getEndpointAuthorizationParameter('id1', 'param1', true);
        assert.ok(data, 'should return a string value');
        assert.equal(data, 'value1', 'should be correct auth param');
        assert.ok(!process.env['ENDPOINT_AUTH_PARAMETER_id1_PARAM1'], 'should clear auth envvar');
    ...
    it('gets undefined if endpoint auth parameter is not set', function (done) {
        ...
        const data = tl.getEndpointAuthorizationParameter('id1', 'noparam', true);
        assert.ok(!data, 'should be undefined when auth param is not set');
    ...
    it('gets required endpoint auth parameters', function (done) {
        ...
        const data = tl.getEndpointAuthorizationParameterRequired('id1', 'param1');
        assert.ok(data, 'should return a string value');
        assert.equal(data, 'value1', 'should be correct auth param');
        assert.ok(!process.env['ENDPOINT_AUTH_PARAMETER_id1_PARAM1'], 'should clear auth envvar');
    ...
    it('throws if endpoint auth parameter is not set', function (done) {
        ...
        try {
            tl.getEndpointAuthorizationParameterRequired('id1', 'noparam');
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'get endpoint authorization parameter should have not have worked');
    ...
    it('gets an endpoint data', function (done) {
        ...
        const data = tl.getEndpointDataParameter('id1', 'param1', true);
        assert.ok(data, 'should return a string value');
    ...
    it('gets undefined if endpoint data is not set', function (done) {
        ...
        const data = tl.getEndpointDataParameter('id1', 'noparam', true);
    ...
    it('gets required endpoint data', function (done) {
        ...
        const data = tl.getEndpointDataParameterRequired('id1', 'param1');
        assert.ok(data, 'should return a string value');
        assert.equal(data, 'val1', 'should be correct object');
    ...
    it('throws if endpoint data is not set', function (done) {
        ...
        try {
            tl.getEndpointDataParameterRequired('id1', 'noparam');
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'get endpoint data should have not have worked');
    ...
    it('gets a secure file name', function (done) {
        ...
        const name = tl.getSecureFileName('10');
    ...
    it('gets a secure file ticket', function (done) {
        ...
        const ticket = tl.getSecureFileTicket('10');
        assert.ok(ticket, 'should return a string value');
        assert.equal(ticket, 'rsaticket10', 'should be correct ticket');
        assert.ok(!process.env['SECUREFILE_TICKET_10'], 'should clear ticket envvar');
    ...
    it('gets case insensitive true bool input value', function (done) {
        ...
        const outVal = tl.getBoolInput('abool', /*required=*/true);
        assert.ok(outVal, 'should return true');
    ...
    it('gets false bool input value', function (done) {
        ...
        const outVal = tl.getBoolInput('abool', /*required=*/true);
        assert.ok(!outVal, 'should return false');
    ...
    it('defaults to false bool input value', function (done) {
        ...
        const outVal = tl.getBoolInput('no_such_env_var', /*required=*/ false);
        assert.ok(!outVal, 'should default to false');
    ...
    it('gets delimited input values removes empty values', function (done) {
        ...
        const inputValue = 'test  value'; // contains two spaces
        process.env['INPUT_DELIM'] = inputValue;
        im._loadData();

        const outVal = tl.getDelimitedInput('delim', ' ', /*required*/true);
    ...
    it('gets delimited input for an empty value', function (done) {
        ...
        const outVal = tl.getDelimitedInput('delim', ' ', /*required*/false);
    ...
    it('gets delimited input with a Regexp', function (done) {
        ...
        const outVal = tl.getDelimitedInput('delim', /[,\n]/, /*required*/false);
    ...
    it('gets path input value', function (done) {
        ...
        const inputValue = 'test.txt'
        process.env['INPUT_PATH1'] = inputValue;
        im._loadData();

        const path = tl.getPathInput('path1', /*required=*/true, /*check=*/false);
    ...
    it('throws if required path not supplied', function (done) {
        ...
        try {
            // @ts-ignore we are testing the behavior of passing null to getPathInput, which is not a valid input name
            tl.getPathInput(null, /*required=*/true, /*check=*/false);
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'req path should have not have worked');
    ...
    it('get invalid checked path throws', function (done) {
        ...
        try {
            const _path = tl.getPathInput('some_missing_path', /*required=*/true, /*check=*/true);
            worked = true;
        } catch (err) { }

        assert.ok(!worked, 'invalid checked path should have not have worked');
    ...
    it('gets path invalid value not required', function (done) {
        ...
        const path = tl.getPathInput('some_missing_path', /*required=*/false, /*check=*/false);
        assert.ok(!path, 'should not return a path');

        const errMsg = errStream.getContents();
    ...
    it('gets path input value with space', function (done) {
        ...
        const inputValue = 'file name.txt';
        const expectedValue = 'file name.txt';
        process.env['INPUT_PATH1'] = inputValue;
        im._loadData();
    ...
    it('gets path value with check and exist', function (done) {
        ...
        const inputValue = __filename;
        process.env['INPUT_PATH1'] = inputValue;
        im._loadData();

        const path = tl.getPathInput('path1', /*required=*/true, /*check=*/true);
        assert.ok(path, 'should return a path');
        assert.equal(path, inputValue, 'test path value');

        const errMsg = errStream.getContents();
        assert.ok(errMsg === "", "no err")
    ...
    it('gets path value with check and not exist', function (done) {
        ...
        const inputValue = "someRandomFile.txt";
        process.env['INPUT_PATH1'] = inputValue;
        im._loadData();
        ...
        try {
            ...
            assert.ok(err.message.indexOf("Not found") >= 0, "error should have said Not found");
        }
    ...
    it('gets path input required value', function (done) {
        ...
        const path = tl.getPathInputRequired('path1', /*check=*/false);
        assert.ok(path, 'should return a path');
        assert.equal(path, inputValue, 'test path value');
    ...
    it('throws if required path not supplied', function (done) {
        ...
        try {
            // @ts-ignore we are testing the behavior of passing null to getPathInputRequired, which is not a valid input name
            tl.getPathInputRequired(null, /*check=*/false);
            worked = true;
        } catch (err) { }
    ...
    it('get required path invalid checked throws', function (done) {
        ...
        try {
            tl.getPathInputRequired('some_missing_path', /*check=*/true);
            worked = true;
        } catch (err) { }
    ...
    it('gets path required value with check and exist', function (done) {
        ...
        const path = tl.getPathInputRequired('path1', /*check=*/true);
        assert.ok(path, 'should return a path');
        assert.equal(path, inputValue, 'test path value');
    ...
    it('filePathSupplied checks not supplied', function (done) {
        ...
        const repoRoot = '/repo/root/dir';
        process.env['INPUT_PATH1'] = repoRoot;
        im._loadData();

        process.env['BUILD_SOURCESDIRECTORY'] = repoRoot;
        const supplied = tl.filePathSupplied('path1');
        assert.ok(!supplied, 'path1 should not be supplied');
        done();
    });
    ...
    it('filePathSupplied checks supplied', function (done) {
        ...
        process.env['BUILD_SOURCESDIRECTORY'] = repoRoot;
        const supplied = tl.filePathSupplied('path1');
        assert.ok(supplied, 'path1 should be supplied');
        done();
    });

    // resolve tests
    it('resolve', function (done) {
        const absolutePath = tl.resolve('/repo/root', '/repo/root/some/path');

        if (os.platform() !== 'win32') {
            assert.ok(absolutePath === '/repo/root/some/path', 'absolute path not expected, got:' + absolutePath + ' expected: /repo/root/some/path');
        } else {
            const winDrive = path.parse(path.resolve('')).root;
            const expectedPath = winDrive.concat('repo\\root\\some\\path');
    ...
    it('gets a task variable', function (done) {
        ...
        const originalAgentVersion = process.env['AGENT_VERSION'];

        try {
            process.env['AGENT_VERSION'] = '2.115.0';
            process.env['VSTS_TASKVARIABLE_TEST1'] = 'Task variable value 1';
            im._loadData();
            const varVal = tl.getTaskVariable('test1');
    ...
    it('sets a task variable', function (done) {
        ...
        try {
            ...
            const varVal = tl.getTaskVariable('test2');
    ...
    it('assert agent does not fail when empty', function (done) {
        ...
        try {
            ...
            assert.ok(!failed, 'assert should not have thrown');
        } finally {
            process.env['AGENT_VERSION'] = originalAgentVersion;
        }
    ...
    it('assert agent fails when less than', function (done) {
        ...
        try {
            ...
            assert.ok(failed, 'assert should have thrown');
        } finally {
            process.env['AGENT_VERSION'] = originalAgentVersion;
        }
    ...
    it('_loadData does not run twice', function (done) {
        ...
        const testDir = path.join(testutil.getTestTemp(), '_loadData-not-called-twice');
        tl.mkdirP(testDir);
