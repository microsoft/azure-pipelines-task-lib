...
describe('Secret Encryption Tests (_exposeTaskLibSecret)', function () {
    ...
    it('encrypts and stores secret with createCipheriv', function () {
        ...
        assert.ok(result.includes(':'), 'Result should contain colon separator');

        const [filePathB64, encryptedContentB64] = result.split(':');
        assert.ok(filePathB64!.length > 0, 'File path should be encoded');
        assert.ok(encryptedContentB64!.length > 0, 'Encrypted content should be encoded');

        // Verify key file was created
        const filePath = Buffer.from(filePathB64!, 'base64').toString();
        assert.ok(fs.existsSync(filePath), 'Key file should exist');

        // Verify key file format (key:iv)
        const keyFileContent = fs.readFileSync(filePath, 'utf8');
        assert.ok(keyFileContent.includes(':'), 'Key file should contain colon separator');

        const [keyBase64, ivBase64] = keyFileContent.split(':');
        assert.ok(keyBase64!.length > 0, 'Key should be present');
        assert.ok(ivBase64!.length > 0, 'IV should be present');
    ...
    it('can decrypt secret encrypted with createCipheriv', function () {
        ...
        // Encrypt
        const result = im._exposeTaskLibSecret(keyFile, secret);
        assert.ok(result, 'Encryption result should not be undefined');
    ...
    it('uses Buffer.from instead of new Buffer', function () {
        ...
        assert.ok(result, 'Result should be valid');
        assert.ok(result.includes(':'), 'Result format should be valid');
    ...
    it('generates unique IV for each encryption', function () {
        ...
        assert.ok(result1, 'First encryption should succeed');
        assert.ok(result2, 'Second encryption should succeed');
    ...
    it('key file format is backward-compatible (key:iv format)', function () {
        ...
        assert.ok(keyFileContent.includes(':'), 'Should use new format with colon separator');
