...
describe('Code Coverage Tests', function () {
    ...
    it('publish code coverage passes all the properties properly', function (done) {
        ...
        const expectedOutput = testutil.buildOutput(["##vso[codecoverage.publish codecoveragetool=Jacoco;summaryfile=\\user\\admin\\summary.xml;reportdirectory=\\user\\admin\\report;additionalcodecoveragefiles=\\user\\admin\\report\\t.xml,\\user\\admin\\report\\c.xml;]"]);
    ...
    it('publish code coverage does not pass properties when the imput parameters are empty', function (done) {
        ...
        const output = stdStream.getContents();
        const expectedOutput = testutil.buildOutput(["##vso[codecoverage.publish]"]);
        assert.equal(expectedOutput, output);
        done();
    });
    ...
    it('publish code coverage does not pass properties when the input parameters are null', function (done) {
        ...
        tl.setStdStream(stdStream);
        const ccPublisher = new tl.CodeCoveragePublisher();
        // @ts-ignore we want to test the null case here, so we are ignoring the type check
    ...
    it('enable code coverage does not pass properties when the input parameters are null', function (done) {
        ...
        tl.setStdStream(stdStream);
        // @ts-ignore we want to test the null case here, so we are ignoring the type check
        const ccEnabler = new tl.CodeCoverageEnabler(null, null);
        const buildProps: { [key: string]: string } = {};
        ccEnabler.enableCodeCoverage(buildProps);

        const output = stdStream.getContents();
        const expectedOutput = testutil.buildOutput(["##vso[codecoverage.enable ]"]);
        assert.equal(expectedOutput, output);
        done();
    });
    ...
    it('enable code coverage passes properties when the input parameters are existing', function (done) {
        ...
        const stdStream = testutil.createStringStream();
        tl.setStdStream(stdStream);
        const ccEnabler = new tl.CodeCoverageEnabler("jacoco", "buildtool");
        const buildProps: { [key: string]: string } = {};
        buildProps['abc'] = 'xyz';
        ccEnabler.enableCodeCoverage(buildProps);

        const output = stdStream.getContents();
        const expectedOutput = testutil.buildOutput(["##vso[codecoverage.enable abc=xyz;buildtool=jacoco;codecoveragetool=buildtool;]"]);
        assert.equal(expectedOutput, output);
        done();
    });
    ...
    it('enable code coverage passes parameters when the input parameters are empty', function (done) {
        ...
        const output = stdStream.getContents();
        const expectedOutput = testutil.buildOutput(["##vso[codecoverage.enable buildtool=jacoco;codecoveragetool=buildtool;]"]);
        assert.equal(expectedOutput, output);
        done();
    });
});
