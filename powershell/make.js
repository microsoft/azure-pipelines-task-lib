...
var run = function(cl) {
    ...
    var rc = exec(cl).code;
    if (rc !== 0) {
        throw new Error('Exec failed with rc ' + rc);
    }
}
...
target.build = function() {
    ...
    // copy the sources
    mkdir('-p', path.join(buildPath, 'VstsTaskSdk'));
    cp('-r', path.join('VstsTaskSdk', '*'), path.join(buildPath, 'VstsTaskSdk'));
...
target.test = function() {
    ...
    mkdir('-p', testPath);
    run(`tsc --outDir ${testPath} --module commonjs --rootDir Tests Tests/lib/psRunner.ts`);
    run(`tsc --outDir ${testPath} --rootDir Tests Tests/L0/_suite.ts`);
    cp('-r', path.join('Tests', '*'), testPath);
    run('mocha ' + path.join(testPath, 'L0', '_suite.js'));
}
